...
class MoE_HyperModel(nn.Module):
    ...
    def forward(self, s: torch.Tensor) -> torch.Tensor:
        ...
        outputs: list[Tensor] = []  # context_dim x [B, 512]
        ...
        return A.reshape(self.f_in, self.r), B.reshape(self.r, self.f_out)  # [context_dim, B, in_f, r], [context_dim, B, r, out_f]
    


class HyperModel(nn.Module) :
    """
    Hypernetwork that generates LoRA adapters.
    lora_dim = (f_in, r, f_out)
    """

    def __init__(self, embedding_dim: int, context_dim: int, lora_dim: tuple[int, int, int]):
        super().__init__()
